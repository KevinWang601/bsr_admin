...
import { getChapterContent, novelVolume } from '@/services/nvoel';

const NovelChapterEdit: React.FC<EditType> = (props) => {
  ...
  const visibleChange = async (visible: boolean) => {
    if (visible) {
      ...
      formRef.current?.setFieldsValue({ content: result.model });
    }
  };

  useEffect(() => {
    if (optionsLoaded && record.parentId) {
      formRef.current?.setFieldsValue({
        parentId: String(record.parentId),
      });
    }
  }, [optionsLoaded, record.parentId]);
      ...
      <ProForm.Group>
        ...
        <ProFormSelect
          ...
          request={async () => {
            ...
            const resp = await novelVolume(record.novelId);
...
export default NovelChapterEdit;
