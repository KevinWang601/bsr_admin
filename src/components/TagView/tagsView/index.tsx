...
const TagsView = (props: { titles: MenuDataItem[]; excludePage: string[] }) => {
  ...
  const getTitle = (path: string) => {
    ...
    for (let i = 0; i < titles.length; i++) {
      ...
      if (title.path === path) return title.name;
      if (title.children === null || title.children?.length === 0) continue;
      for (let j = 0; j < (title.children?.length || 0); j++) {
        const child = title.children![j];
